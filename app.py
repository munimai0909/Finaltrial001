...
def chat():
    ...
    if "invoice" in user_input:
        ...
        invoice_path = f"invoices/{username}/invoice-001.pdf"
        os.makedirs(os.path.dirname(invoice_path), exist_ok=True)
        invoice.generate_invoice(invoice_data, invoice_path)
        response = f"Invoice created! [Download Invoice]({invoice_path})"
        ...
        response = "Try 'Add invoice', 'Show ledger', etc."
